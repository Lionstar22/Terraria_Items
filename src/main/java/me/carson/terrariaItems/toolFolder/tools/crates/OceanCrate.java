...
import me.carson.terrariaItems.accesoryFolder.accessories.Flipper;
...
public class OceanCrate extends Tool {
    ...
    public void rightActivate(Player player) {
        ...
        //treasure loot table
        int treasure = ThreadLocalRandom.current().nextInt(1, 2);
        switch (treasure){
            case 1 -> player.getInventory().addItem(Flipper.getItem(plugin));
            default -> {
                return;
            }
        }
    ...
    public static ItemStack getItem(Plugin plugin) {
        ItemStack item=new OceanCrate(plugin).createItem();
